...
export async function execute(interaction: CommandInteraction): Promise<void> {
  try {
    ...
    await interaction.editReply(
      Formatters.formatSuccessMessage(
        `Voting ended! Player ${votedPlayer.playerNumber} received the most votes (${voteResults[0].voteCount}). Check your DMs for details.`
      )
    );
