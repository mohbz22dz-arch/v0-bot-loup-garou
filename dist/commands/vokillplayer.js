...
export async function execute(interaction) {
    try {
        ...
        if (!Validators.validatePlayerNumber(playerNumber, session.players.length)) {
            await interaction.editReply(Formatters.formatErrorMessage(`Invalid player number. Valid range: 1-${session.players.length}`));
            return;
        }
        await voteService.killPlayer(session.id, playerNumber);
        await interaction.editReply(Formatters.formatSuccessMessage(`Player ${playerNumber} has been eliminated.`));
...
//# sourceMappingURL=vokillplayer.js.map