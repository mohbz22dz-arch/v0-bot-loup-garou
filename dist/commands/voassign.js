...
export async function execute(interaction) {
    try {
        ...
        if (!roleCounts) {
            await interaction.editReply(Formatters.formatErrorMessage('Invalid role format. Use: "2 Loup-Garou, 1 Voyante, 2 Sorcière, 1 Chasseur, 3 Villageois"'));
            return;
        }
        ...
        if (!Validators.validateRoleCounts(roleCounts, session.players.length)) {
            await interaction.editReply(Formatters.formatErrorMessage(`Role count (${Object.values(roleCounts).reduce((a, b) => a + b, 0)}) doesn't match player count (${session.players.length})`));
            return;
        }
        const report = await roleService.assignRoles(session.id, roleCounts, session.players, async (userId) => interaction.client.users.fetch(userId).catch(() => null));
        ...
        await interaction.editReply(Formatters.formatSuccessMessage('Roles assigned! Check your DMs for the role assignment report.'));
    }
    catch (error) {
...
//# sourceMappingURL=voassign.js.map